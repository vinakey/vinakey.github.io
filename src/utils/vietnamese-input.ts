...
import { AnvimEngine, type VietnameseInputMethod } from "@/engine/methods/anvim";
...
class VietnameseInput {
  ...
  private handleInputWithProcessing(
    element: HTMLElement,
    event: Event,
  ): void {
    if (!this.enabled || this.currentMethod === "OFF") return;
  ...
  private processCurrentWord(
    ...
    if (prefix.length === 0 && /[\p{L}]/u.test(lastKey) === false) {
      // No word context and key isn't a letter → nothing to process
      return;
    }

    this.anvimEngine.setMethodByString(this.currentMethod as VietnameseInputMethod);
    ...
    if (processedWord !== currentWord) {
      const beforeWord = priorText.substring(0, priorText.length - prefix.length);
...
if (typeof window !== "undefined") {
  ...
  console.log(
    "🌐 vietnameseInput is now available in console!",
  );
}
