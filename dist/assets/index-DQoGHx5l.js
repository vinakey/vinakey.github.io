...
const K = {
  now: () => (
    Z === void 0 &&
      K.set(
        J.isProcessing || Tt.useManualTiming ? J.timestamp : performance.now(),
      ),
    Z
  ),
  set: (t) => {
...
const Ve = { test: ke("#"), parse: Ts, transform: B.transform },
  ...
  x = {
    ...
    parse: (t) =>
      B.test(t) ? B.parse(t) : _.test(t) ? _.parse(t) : Ve.parse(t),
    transform: (t) =>
      typeof t == "string"
        ? t
...
function Jt(t, e) {
  let n = _e(t);
  return (
    n !== Ae && (n = W), n.getAnimatableNone ? n.getAnimatableNone(e) : void 0
  );
}
...
function ti({
  ...
  const a = 5 / t,
    l = si(i, r, a);
  if (((t = R(t)), isNaN(l)))
    return { stiffness: S.stiffness, damping: S.damping, duration: t };
  {
...
function hn(t = S.visualDuration, e = S.bounce) {
  ...
  if (v < 1) {
    ...
    V = (A) => {
      ...
      return (
        o - w * (((b + v * g * y) / m) * Math.sin(m * A) + y * Math.cos(m * A))
      );
    };
    ...
    V = (A) => {
      const w = Math.exp(-v * g * A),
        p = Math.min(m * A, 300);
      return (
        o - (w * ((b + v * g * y) * Math.sinh(p) + m * y * Math.cosh(p))) / m
      );
    };
  }
  const M = {
...
class ct extends an {
  ...
  initPlayback(e, n) {
    ...
    if (
      (typeof r == "string" && Q() && Pi(r) && (r = fn[r]), wi(this.options))
    ) {
      const {
          onComplete: h,
          onUpdate: d,
    ...
    const c = Vi(a.owner.current, l, e, {
      ...this.options,
      duration: s,
      times: i,
      ease: r,
...
function pn(t, e, { delay: n = 0, transitionOverride: s, type: i } = {}) {
  ...
  for (const h in l) {
    ...
    cs(t, h),
      d.start(
        Ri(h, d, f, t.shouldReduceMotion && Nt.has(h) ? { type: !1 } : b, t, v),
      );
    const y = d.animation;
    y && u.push(y);
  }
...
function Gi(t) {
  return (e) =>
    Promise.all(e.map(({ animation: n, options: s }) => Bi(t, n, s)));
}
function Ui(t) {
  let e = Gi(t),
    n = ht(),
  ...
  function o(l) {
    ...
    for (let y = 0; y < _i; y++) {
      ...
      const { prevResolvedValues: We = {} } = T,
        ...
        ze = (P) => {
          (p = !0),
            d.has(P) && ((C = !0), d.delete(P)),
            (T.needsAnimating[P] = !0);
      ...
      (T.prevProp = V),
        ...
        p &&
          (!(A && w) || C) &&
          h.push(...F.map((P) => ({ animation: P, options: { type: g } })));
    }
    if (d.size) {
      const y = {};
      d.forEach((g) => {
...
class Zi extends z {
  mount() {
    ...
    e &&
      (this.unmount = Xn(
        e,
        (n) => (pt(this.node, n, "Start"), (s) => pt(this.node, s, "End")),
      ));
  }
  unmount() {}
}
...
class dr {
    ...
    (this.current = null),
      ...
      (this.scheduleRender = () => {
        ...
        this.renderScheduledAt < f &&
          ((this.renderScheduledAt = f), D.render(this.render, !1, !0));
      });
  ...
  getValue(e, n) {
    ...
    return (
      s === void 0 &&
        n !== void 0 &&
        ((s = ee(n === null ? void 0 : n, { owner: this })),
        this.addValue(e, s)),
      s
    );
  }
...
const gr = (t, e) =>
    _n(t) ? new mr(e) : new pr(e, { allowProjection: t !== Gn.Fragment }),
  vr = { renderer: gr, ...Hi, ...ar };
var Vr = vr;
export { Vr as default };
//# sourceMappingURL=index-DQoGHx5l.js.map
